...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # API Provider selection - OpenAI recommended
        provider = st.selectbox(
            "Select AI Provider",
            ["Demo (No API)", "OpenAI (Recommended)", "Google Gemini"],
            index=0 if not ai_service.is_connected else (1 if ai_service.provider == "openai" else 2)
        )
        
        if provider == "OpenAI (Recommended)":
            st.markdown('<small style="color: #28a745;">⭐ Best choice - Fast & Reliable</small>', unsafe_allow_html=True)
            api_key = st.text_input("OpenAI API Key", type="password", key="openai_key")
            if st.button("🚀 Connect OpenAI", use_container_width=True, type="primary"):
                if api_key:
                    with st.spinner("Connecting to OpenAI..."):
                        success, message = ai_service.configure_openai(api_key)
            ...
            st.caption("[Get API key from OpenAI](https://platform.openai.com/api-keys)")
