...
class AIService:
    ...
    def configure_groq(self, api_key: str) -> tuple:
        """Configure Groq API"""
        try:
            ...
            if response.choices[0].message.content:
                ...
                self.is_connected = True
                return True, "✅ Connected to Groq (Llama 3.3 70B)"
                
        except ImportError:
            return False, "❌ groq not installed. Using Demo Mode."
        except Exception as e:
            return False, f"❌ Groq error: {str(e)[:50]}. Using Demo Mode."
        
        return False, "❌ Could not connect to Groq. Using Demo Mode."
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if ai_service.is_connected:
            st.markdown(f"""
                <div class="api-status-ok">
                    ✅ {ai_service.provider.upper()}: {ai_service.model_name}
                </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
        ...
        provider = st.selectbox(
            "Select AI Provider",
            ["Demo (No API)", "Google Gemini", "Groq"],
            index=0 if not ai_service.is_connected else (1 if ai_service.provider == "gemini" else 2)
        )
        ...
        if provider == "Google Gemini":
            ...
            if st.button("Connect Gemini", use_container_width=True):
                if api_key:
                    with st.spinner("Connecting..."):
                        success, message = ai_service.configure_gemini(api_key)
                        if success:
                            st.success(message)
                        else:
                            st.warning(message)
            ...
            if st.button("Connect Groq", use_container_width=True):
                if api_key:
                    with st.spinner("Connecting..."):
                        success, message = ai_service.configure_groq(api_key)
                        if success:
                            st.success(message)
                        else:
                            st.warning(message)
                        st.rerun()
            st.caption("[Get free API key](https://console.groq.com/keys)")
        
        else:
            st.info("🎮 Demo Mode: All features work with sample responses. Connect an API for AI-powered results!")
